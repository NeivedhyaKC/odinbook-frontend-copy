...
const PostCard = (props) => {
  ...
  const onLikeButtonClick = async () => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/users/${user._id}/posts/${props.post._id}/like`,
      {
        method: "PUT",
        credentials: "include",
  ...
  const onPostCommentClick = async (event) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/users/${user._id}/posts/${props.post._id}/comment`,
      {
        method: "PUT",
        credentials: "include",
        withCredentials: true,
  ...
  return (
    <div className={`${showClass ? "postCard show" : "postCard"}`}>
      <ProfileCard
        profileCardPicWidth="max(8%,40px)"
        paddingLeft="2%"
        profileCardNamePaddingLeft="2%"
        profileCardSubnamePaddingLeft="2%"
        profileCardSubnamePaddingTop="1%"
        profileCardNameFontSize={20}
        postedAt={props.post.postedAt}
        firstName={props.post.userId.firstName}
        lastName={props.post.userId.lastName}
      ...
      {props.post.photoUrl ? (
        <img
          src={`${process.env.REACT_APP_API_URL}/users/${props.post.userId._id}/posts/${props.post._id}/image`}
          alt="postImage"
          className="postImage"
        />
      ) : null}
      ...
      >
        <div className="commentCard">
          <div className="commentCardChild">
            {comments
              ? comments.map((commentFromCommentsArray) => {
                  return (
                    <ProfileCard
                      profileCardPicWidth="7%"
                      profileCardSubnamePaddingTop="1%"
                      profileCardSubnamePaddingLeft="2%"
                      profileCardNamePaddingLeft="2%"
                      paddingLeft="1%"
                      ...
                      userId={commentFromCommentsArray.userId._id}
                      comment={commentFromCommentsArray.comment}
                      firstName={commentFromCommentsArray.userId.firstName}
                      lastName={commentFromCommentsArray.userId.lastName}
                      key={uuidv4()}
                    />
                  );
                })
